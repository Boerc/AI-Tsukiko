...
export type GoogleConfig = { projectId: string; location: string };

export class GoogleAI {
  private ttsClient: TextToSpeechClient;
  private sttClient: SpeechClient;
  private vertex: VertexAI | null;
  ...
  async synthesizeTextToSpeech(text: string, voiceName = 'en-US-Neural2-F', speakingRate = 1.05): Promise<Buffer> {
    ...
    if (!audio) throw new Error('No audio content from TTS');
    return Buffer.from(audio as Buffer | Uint8Array);
  }

  async transcribeShortAudio(buffer: Buffer, languageCode = 'en-US'): Promise<string> {
    ...
    const transcript = result.results?.map((r: any) => r.alternatives?.[0]?.transcript ?? '').join(' ').trim() ?? '';
    return transcript;
  }
